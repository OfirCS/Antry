...
import { ArrowLeft, Trophy, Users, Zap, LogOut, Send } from "lucide-react";
import { getInitials } from "@/lib/mock-data";
import { joinHackathon, leaveHackathon, submitToHackathon } from "../../actions";
...
export default function HackathonDetailClient({
  ...
  const [submitState, submitAction, isSubmitting] = useActionState<FormState, FormData>(submitToHackathon, null);
  ...
  return (
    <div className="bg-background-primary min-h-screen">
          ...
          <div className="ml-auto flex items-center gap-3">
            {!isLoggedIn ? (
              <Link
                ...
                className="px-6 py-2.5 bg-accent text-white rounded-lg font-semibold text-[13px] hover:opacity-90 transition-opacity"
              >
                Sign in to join
              </Link>
            ) : hasJoined ? (
              <>
                {event.status === "active" && availableProjects.length > 0 && (
                  <button
                    type="button"
                    onClick={() => setShowSubmitForm(!showSubmitForm)}
                    className="px-6 py-2.5 bg-accent text-white rounded-lg font-semibold text-[13px] hover:opacity-90 transition-opacity inline-flex items-center gap-2"
                  >
                ...
                <form action={leaveHackathon}>
                  <input type="hidden" name="hackathon_id" value={event.id} />
                  <button
                    type="submit"
                    className="px-5 py-2.5 border border-border-primary rounded-lg font-semibold text-[13px] text-text-secondary hover:text-red-500 hover:border-red-500/30 transition-colors inline-flex items-center gap-2"
                  >
              ...
              <form action={joinHackathon}>
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 bg-accent text-white rounded-lg font-semibold text-[13px] hover:opacity-90 transition-opacity active:scale-[0.97]"
                >
                  Join hackathon
                </button>
              </form>
      ...
      {showSubmitForm && hasJoined && (
        <section className="border-b border-border-primary bg-background-secondary/30">
          <div className="max-w-[900px] mx-auto px-8 py-6">
            ...
            <form action={submitAction} className="flex items-center gap-4">
              ...
              <select
                name="project_id"
                required
                className="flex-1 max-w-sm px-4 py-2.5 bg-surface border border-border-primary rounded-lg text-[13px] text-text-primary outline-none focus:border-accent/40 transition-colors"
              >
              ...
              <button
                ...
                className="px-6 py-2.5 bg-accent text-white rounded-lg font-semibold text-[13px] hover:opacity-90 transition-opacity disabled:opacity-50"
              >
                {isSubmitting ? "Submitting..." : "Submit"}
              </button>
            </form>
            {submitState?.error && (
              <p className="text-red-500 text-[13px] mt-2">{submitState.error}</p>
            )}
            {submitState?.success && (
              <p className="text-green-600 text-[13px] mt-2">Project submitted successfully!</p>
            )}
          </div>
        </section>
      )}
        ...
        <div className="max-w-[900px] mx-auto">
          <div className="flex items-baseline justify-between mb-10">
            ...
            <span className="text-[12px] font-medium text-text-tertiary tracking-wide">
              {event.submissions.length} project{event.submissions.length !== 1 && "s"}
            </span>
          </div>
