...
export default function ProjectDetailPage() {
  ...
  if (!project) {
    return (
      <div className="max-w-[800px] mx-auto px-8 py-20 text-center">
        ...
        <Link
          href="/discover"
          className="text-[13px] text-accent mt-3 inline-block"
        >
          Back to discover
        </Link>
      </div>
  ...
  return (
    <div
      className={
        expanded
          ? "fixed inset-0 z-50 bg-background-primary overflow-auto pt-[72px]"
          : ""
      }
    >
      <div
        className={
          expanded
            ...
            : "max-w-[900px] mx-auto px-6 sm:px-8 py-10 md:py-16"
        }
      >
        {/* Top bar */}
        <div
          className={
            expanded
              ? "flex items-center justify-between px-6 py-3 border-b border-border-primary shrink-0"
              : "flex items-center justify-between mb-8"
          }
        >
          <Link
            href="/discover"
            className="inline-flex items-center gap-1.5 text-[12px] text-text-tertiary hover:text-text-primary transition-colors font-medium"
          >
          ...
          <div className="flex items-center gap-2">
            {project.demoUrl && (
              <a
                ...
                className="inline-flex items-center gap-1.5 text-[11px] text-text-tertiary hover:text-text-primary transition-colors font-medium"
              >
            ...
            {project.demoUrl && (
              <button
                ...
                className="inline-flex items-center gap-1.5 text-[11px] text-text-tertiary hover:text-text-primary transition-colors font-medium ml-3"
              >
                {expanded ? (
                  <>
        ...
        {project.demoUrl && (
          <div
            className={
              expanded
                ? "flex-1 min-h-0"
                : "aspect-video rounded-lg overflow-hidden border border-border-primary mb-8 shadow-sm"
            }
          >
            <iframe
              src={project.demoUrl}
        ...
        {/* If no demo URL, show gradient header */}
        {!project.demoUrl && (
          <motion.div
        ...
        {/* Project info — hidden when fullscreen */}
        {!expanded && (
          <>
            {/* Title & meta */}
            <motion.div
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              {project.demoUrl && (
                ...
                >
                  Open demo <ArrowUpRight className="w-3.5 h-3.5" />
                </a>
              )}
              {project.sourceUrl && (
                <a
