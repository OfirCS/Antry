...
function MockProfileCard() {
  return (
    <div className="rounded-lg border border-[#E5E7EB] bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.06)]">
      <div className="flex items-start gap-4 mb-6">
        <div className="w-12 h-12 rounded-full bg-[#111111] flex items-center justify-center text-[14px] font-semibold text-[#20F5A0] shrink-0">
          MC
        </div>
        <div>
          <h4 className="text-[16px] font-semibold text-[#111111] tracking-tight">Mara Chen</h4>
          <p className="text-[13px] text-[#4B5563] leading-snug mt-1">AI engineer building things that think.</p>
        </div>
      </div>
      <div className="flex flex-wrap gap-2 mb-6">
        {["Python", "LangChain", "TypeScript", "RAG"].map((s) => (
          <span
            key={s}
            className="inline-flex items-center rounded-full bg-[#F3F4F6] px-2.5 py-0.5 text-[10px] font-medium text-[#4B5563]"
          >
            {s}
          </span>
        ))}
      ...
      <div className="grid grid-cols-3 gap-4 pt-6 border-t border-[#E5E7EB]">
        {[
          ...
          <div key={m.label}>
            ...
            <p className="text-[10px] font-semibold uppercase tracking-widest text-[#6B7280] mt-1">{m.label}</p>
          </div>
        ))}
      </div>
    </div>
...
function MockProjectCard({ title, desc, likes }: { title: string; desc: string; likes: number }) {
  return (
    <div className="rounded-lg border border-[#E5E7EB] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.06)]">
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          <h4 className="text-[14px] font-semibold text-[#111111] tracking-tight">{title}</h4>
          <p className="text-[12px] text-[#4B5563] mt-1 leading-relaxed">{desc}</p>
        </div>
        <motion.span
          className="flex items-center gap-1.5 text-[#6B7280] text-[11px] font-medium shrink-0"
...
function MockDiscoverCard({ name, initials, tagline }: { name: string; initials: string; tagline: string }) {
  return (
    <div className="group/discover cursor-pointer flex items-center gap-3 rounded-lg border border-[#E5E7EB] bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.06)] transition-all duration-200 hover:shadow-[0_4px_16px_rgba(0,0,0,0.06),0_1px_4px_rgba(0,0,0,0.04)] hover:-translate-y-[1px]">
      <div className="w-8 h-8 rounded-full bg-[#F3F4F6] flex items-center justify-center text-[10px] font-semibold text-[#4B5563] group-hover/discover:bg-[#111111] group-hover/discover:text-[#20F5A0] transition-colors duration-200">
        {initials}
      </div>
      <div className="min-w-0">
        <h4 className="text-[12px] font-semibold text-[#111111] tracking-tight">{name}</h4>
        <p className="text-[11px] text-[#6B7280] mt-0.5 truncate">{tagline}</p>
      </div>
    </div>
  );
}
...
function FloatingDot({ className, delay = 0 }: { className?: string; delay?: number }) {
  return (
    <motion.div
      className={`absolute w-2 h-2 rounded-full bg-[#20F5A0]/20 ${className ?? ""}`}
      animate={{
        y: [0, -8, 0],
        opacity: [0.3, 0.7, 0.3],
      }}
...
export function PlatformPreview() {
  return (
    <section className="bg-[#F7F8FA] overflow-hidden">
      <div className="max-w-[1240px] mx-auto px-6 sm:px-10 py-24 sm:py-32">
        <motion.div
          ...
          className="mb-16"
        >
          <p className="inline-flex items-center gap-2 text-[13px] font-semibold tracking-[0.06em] uppercase text-[#6B7280] mb-4">
            System Interface
          </p>
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-12 gap-5">
          ...
          <motion.div
            className="hidden md:block absolute w-3 h-3 rounded-full border border-[#20F5A0]/30 top-[8%] right-[15%] z-10"
          ...
          <div className="pointer-events-none absolute inset-y-0 -left-1 w-12 z-20 bg-gradient-to-r from-[#F7F8FA] to-transparent" />
          {/* Gradient overlay at right edge for depth */}
          <div className="pointer-events-none absolute inset-y-0 -right-1 w-12 z-20 bg-gradient-to-l from-[#F7F8FA] to-transparent" />
          {/* Gradient overlay at bottom edge for depth */}
          <div className="pointer-events-none absolute -bottom-1 inset-x-0 h-12 z-20 bg-gradient-to-t from-[#F7F8FA] to-transparent" />
          ...
          <BentoItem
            ...
            parallaxOffset={12}
          >
            <div className="rounded-lg bg-[#EEF6FF] p-8 flex flex-col justify-center h-full">
              <MockProfileCard />
              <p className="mt-8 text-[14px] leading-relaxed text-[#4B5563]">
          ...
          <div className="md:col-span-7 flex flex-col gap-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <BentoItem index={1} parallaxOffset={8}>
                <div className="rounded-lg bg-[#ECFDF5] p-6">
              ...
              <BentoItem index={2} parallaxOffset={16}>
                <div className="rounded-lg bg-[#F5F3FF] p-6">
            ...
            <BentoItem index={3} parallaxOffset={6} className="flex-1">
              <div className="rounded-lg bg-[#EFF6FF] p-8 h-full">
                <div className="flex items-center justify-between mb-6">
                  <p className="inline-flex items-center gap-2 text-[13px] font-semibold tracking-[0.06em] uppercase text-[#6B7280]">
                    Discovery Feed
                  </p>
                  <motion.div
